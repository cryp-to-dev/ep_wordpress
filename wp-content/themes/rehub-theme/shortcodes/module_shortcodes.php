		...
		if(!empty($atts['offer_coupon'])) :
			...
			if (empty($atts['offer_coupon_mask'])) :
                ...
                $out .= '<div class="mt15 rehub_offer_coupon free_coupon_width masked_coupon ';
           		...
           		$out .= '" data-clipboard-text="'.rawurlencode(esc_html($atts['offer_coupon'])).'" data-codetext="'.rawurlencode(esc_html($atts['offer_coupon'])).'" data-dest="'.esc_url($atts['button_link']).'">'.$offer_coupon_mask_text.'<i class="rhicon rhi-external-link-square"></i></div>';
			endif;	
		endif;
        if(!empty($atts['offer_coupon_date'])) :
